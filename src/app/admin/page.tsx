...
import { ShieldAlert, Bike as BikeIconLucide, ListChecks, CalendarClock, PlusCircle, Pencil, Trash2, Users, Clock, MapPin, DollarSign } from 'lucide-react';
...
interface EnrichedRental extends Rental {
  renterName?: string;
  bikeName?: string; // Already in Rental type, but ensuring it for display
}
...
export default function AdminPage() {
  ...
  const activeRentals: EnrichedRental[] = useMemo(() => {
    return allRentals
      ...
      .sort((a,b) => new Date(a.endDate).getTime() - new Date(b.endDate).getTime()); // Sort by soonest return
  ...
  const upcomingAppointments: EnrichedRental[] = useMemo(() => {
    return allRentals
      ...
      .sort((a,b) => new Date(a.startDate).getTime() - new Date(b.startDate).getTime()); // Sort by soonest start
  }, [allRentals, allUsers, bikes]);
  ...
  const handleDeleteBikeConfirm = (bikeId: string) => {
    ...
    setBikeToDelete(null); // Close dialog
  };
  ...
  const handleBikeFormSubmit = async (data: BikeFormData, bikeIdToUpdate?: string) => {
    ...
    const bikeDataFromForm: Omit<Bike, 'id' | 'rating'> & { rating?: number | null } = {
      ...
      rating: data.rating === null ? undefined : data.rating, // Handle null from form
    };
    
    if (bikeIdToUpdate) { // Editing existing bike
      ...
      const newBike: Bike = {
        ...bikeDataFromForm,
        id: `bike${Date.now()}-${Math.random().toString(36).substring(2,7)}`, // More unique ID
        rating: bikeDataFromForm.rating, // rating is already optional in Bike type
      };
  ...
  return (
    <MainLayout>
      ...
      {bikeToDelete && (
        <AlertDialog open={!!bikeToDelete} onOpenChange={() => setBikeToDelete(null)}>
            <AlertDialogContent>
                <AlertDialogHeader>
                ...
                <AlertDialogFooter>
                <AlertDialogCancel onClick={() => setBikeToDelete(null)}>Cancel</AlertDialogCancel>
